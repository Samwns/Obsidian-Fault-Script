...
Section "Install"
	SetOutPath "$INSTDIR"
	DetailPrint "Installing OFS self-hosted compiler"
	File "${DIST_DIR}\ofscc.exe"
	DetailPrint "Installing command wrappers"
	File /nonfatal "${DIST_DIR}\ofs.cmd"
	File /nonfatal "${DIST_DIR}\install.cmd"
	...
	File /nonfatal "${DIST_DIR}\install.ps1"
	...
	DetailPrint "Installing OFS compiler sources"
	SetOutPath "$INSTDIR\ofscc"
	File /nonfatal "${DIST_DIR}\ofscc\*.ofs"
	File /nonfatal "${DIST_DIR}\ofscc\README.md"
	SetOutPath "$INSTDIR"
...
Section "Uninstall"
	Delete "$INSTDIR\ofscc.exe"
	Delete "$INSTDIR\ofs.cmd"
