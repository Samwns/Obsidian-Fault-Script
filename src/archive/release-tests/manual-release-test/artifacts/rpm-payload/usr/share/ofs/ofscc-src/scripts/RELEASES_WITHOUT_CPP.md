...
bash src/scripts/release-native-only.sh 1.0.0
...
**Opções:**
```bash
EXISTING_COMPILER=ofs/dist/ofscc bash bootstrap-minimal.sh
...
2. Rodar: `bash src/scripts/bootstrap-minimal.sh`
3. Pronto!
...
bash src/scripts/release-native-only.sh 1.0.0-$(date +%Y%m%d)

# Com recompilação automática
bash src/scripts/release-native-only.sh 1.0.0 --recompile
```
...
bash src/scripts/release-native-only.sh $VERSION --recompile
...
nano ofs/ofs/bootstrap/ofscc.ofs

# Test change
bash src/scripts/bootstrap-minimal.sh

# Works? Create release
bash src/scripts/release-native-only.sh 1.0.1 --recompile
...
# 2. Create src/packages
bash ofs/bootstrap/scripts/release-native-only.sh "$VERSION" --recompile
...
**Use:**
```bash
bash src/scripts/auto-release.sh 1.0.0
