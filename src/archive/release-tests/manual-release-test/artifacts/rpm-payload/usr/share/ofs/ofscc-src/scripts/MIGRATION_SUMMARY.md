...
─────────────────────────
Copy ofscc_v3 → ofs/dist/ofscc
Copy stdlib → ofs/dist/stdlib/
...
```
ofs/bootstrap/ofscc.ofs          ~80 linhas (driver)
ofs/bootstrap/lexer.ofs          ~660 linhas
ofs/bootstrap/parser.ofs         ~650 linhas
ofs/bootstrap/typeck.ofs         ~350 linhas
ofs/bootstrap/codegen.ofs        ~400 linhas
...
```bash
# Compilar arquivo
ofs/dist/ofscc build meu_programa.ofs -o meu_programa

# Type-check
ofs/dist/ofscc check meu_programa.ofs

# Inspecionar tokens
ofs/dist/ofscc tokens meu_programa.ofs

# Inspecionar AST
ofs/dist/ofscc ast meu_programa.ofs
```
...
**Linux/macOS:**
```bash
./src/scripts/utils.sh compile hello.ofs -o hello
./src/scripts/utils.sh test
./src/scripts/utils.sh version
```
...
| "ofscc_v2 failed" | ofscc.ofs não encontrado | Verificar `ofs/ofs/bootstrap/ofscc.ofs` |
| "Permission denied" | Scripts não executáveis | `chmod +x src/scripts/*.sh` |
...
1. **Validar instalação:**
   ```bash
   bash src/scripts/validate.sh -v
   ```
