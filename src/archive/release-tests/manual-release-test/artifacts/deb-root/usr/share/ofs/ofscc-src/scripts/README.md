...
│ ──────────────────────────────────────────────────────────  │
│ • Copiar ofscc_v3 → ofs/dist/ofscc (compilador final)         │
│ • Copiar runtime library → ofs/dist/libofs_runtime.a           │
│ • Copiar stdlib → ofs/dist/stdlib/                             │
│ • Criar version.json                                       │
└─────────────────────────────────────────────────────────────┘
```
...
- `ofs/dist/libofs_runtime.a` — biblioteca runtime
- `ofs/dist/stdlib/` — Bibliotecas padrão
...
ofs/dist/ofscc build program.ofs -o program

# Type-check sem gerar binário
ofs/dist/ofscc check program.ofs

# Ver tokens (debug)
ofs/dist/ofscc tokens program.ofs

# Ver AST
ofs/dist/ofscc ast program.ofs

# Ver LLVM IR
ofs/dist/ofscc ir program.ofs

# Ver assembly nativo
ofs/dist/ofscc asm program.ofs
```
...
```bash
bash src/scripts/create-release.sh 1.0.0-native
```
...
- Verifique se `ofs/ofs/bootstrap/ofscc.ofs` existe e é válido
...
2. **Verificar stdlib:**
   ```bash
   ofs/dist/ofscc check ofs/examples/collections.ofs
   ofs/dist/ofscc check ofs/examples/canvas_window_demo.ofs
   ```
...
# ✓ stdlib completa
ls -la ofs/dist/stdlib/

# ✓ Exemplos compilam
for ex in ofs/examples/{hello,calculator,fizzbuzz}.ofs; do
  ofs/dist/ofscc build $ex && echo "✓ $(basename $ex)"
done
...
# ✓ Determinismo
bash ofs/ofs/bootstrap/test_bootstrap.sh
```
