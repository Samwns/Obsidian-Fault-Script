...
```bash
./src/packages/install_library.sh src/packages/dist/window.tar.gz
```

Windows PowerShell:

```powershell
./src/packages/install_library.ps1 -Package src/packages/dist/window.tar.gz
```
...
```bash
./src/packages/uninstall_library.sh window
```

Windows PowerShell:

```powershell
./src/packages/uninstall_library.ps1 -LibraryName window
```
...
- The default user src/packages directory is searched automatically.
- `OFS_LIB_PATH` is only needed when you want extra custom search paths.

### Current src/packages in the repository
