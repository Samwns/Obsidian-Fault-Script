...
release                Generate release src/packages
...
```bash
# Compilar programa
bash src/scripts/utils.sh compile my_program.ofs -o my_program

# Verificar tipos
bash src/scripts/utils.sh check my_program.ofs
...
bootstrap-windows.ps1/linux.sh  ← Scripts principais
    ↓ ↓
    ├→ cria ofs/dist/ofscc
    ├→ cria ofs/dist/stdlib/
    └→ cria dist/version.json

utils.ps1/sh  ← Usa ofs/dist/ofscc para operações
    ├→ compile
    ├→ check
    ├→ tokens/ast/ir/asm
    └→ test
