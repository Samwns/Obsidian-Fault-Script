...
if [ ! -f "ofs/dist/ofscc" ]; then
    print_error "Compiler not found: ofs/dist/ofscc"
fi

if [ ! -x "ofs/dist/ofscc" ]; then
    chmod +x ofs/dist/ofscc || print_error "Cannot make compiler executable"
fi
...
if [ ! -d "ofs/dist/stdlib" ]; then
    print_error "Standard library not found: ofs/dist/stdlib"
fi

STDLIB_COUNT=$(find ofs/dist/stdlib -name "*.ofs" 2>/dev/null | wc -l)
...
if [ ! -f "ofscc_v2" ] || [ ! -f "ofscc_v3" ]; then
    ...
    if bash ofs/ofs/bootstrap/test_bootstrap.sh > /dev/null 2>&1; then
...
if [ ! -f "$TEST_FILE" ]; then
    ...
    if ofs/dist/ofscc build "$TEST_FILE" -o "$TEST_OUT" -O2 > /dev/null 2>&1; then
...
for cmd in "--help" "--version" "tokens --help" "ast --help"; do
    if ofs/dist/ofscc $cmd > /dev/null 2>&1 || ofs/dist/ofscc $cmd 2>&1 | grep -q "usage\|Usage\|help\|Help"; then
        print_success "Command works: ofs/dist/ofscc $cmd"
    else
        print_warn "Command may not work: ofs/dist/ofscc $cmd"
    fi
done
...
echo ""
echo "Next steps:"
echo "  • Try: ofs/dist/ofscc build ofs/examples/hello.ofs -o hello"
echo "  • Run: ./hello"
echo "  • Check: ofs/dist/ofscc check ofs/examples/calculator.ofs"
echo "  • Test: ./src/scripts/utils.sh test"
echo ""
