...
# Saída: ofs/dist/ofscc (compilador fresco)
```
...
- run: bash ofs/bootstrap/scripts/release-native-only.sh $VERSION --recompile
...
```bash
bash ofs/bootstrap/scripts/bootstrap-minimal.sh
bash ofs/bootstrap/scripts/release-native-only.sh 1.0.0 --test
```
