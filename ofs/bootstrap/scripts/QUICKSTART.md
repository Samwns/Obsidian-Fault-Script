...
# 3. Compilar um exemplo
bash src/scripts/utils.sh compile ofs/examples/hello.ofs -o hello
./hello
```
...
5. **Criar release**:
   ```bash
   bash src/scripts/create-release.sh 1.0.0
   ```
...
| "Permission denied" | Execute `chmod +x src/scripts/*.sh` |
...
```bash
# Debug build
bash src/scripts/bootstrap-linux.sh --debug --jobs 4

# Com LTO (Link-Time Optimization)
ENABLE_LTO=true bash src/scripts/bootstrap-linux.sh

# In-place build
BUILD_DIR=/tmp/ofs bash src/scripts/bootstrap-linux.sh
```

### Releases customizadas
```bash
# Custom version
bash src/scripts/create-release.sh my-version-1.5.0

# Custom install dir
INSTALL_DIR=/mylibs bash src/scripts/bootstrap-linux.sh
```
